...
function draw() {
	...
	stroke(0);
	textFont(monoFont);
  text( removeDiacritics(prose.toUpperCase()), x + width * 0.1, y + height * 0.05, w - width * 0.05, h - height * 0.05);
...
function drawStar(){
	  //branch(40.0, 0.23, 0.0);
	  branchCount = 0;
	  // 5 max 
	  branch(40.0, map(mouseX, 0, width, 0, 3.14 ), 5); //Math.floor(map(mouseX, 0, width, 0, 7)));
...
function branch(dist, theta, n){
   ...
   for (var i = 0; i < n; i++) {
    ...
    translate(0, -dist);
    branch(dist * 0.55, -theta * 3.2, n - 1);
    pop();
  }

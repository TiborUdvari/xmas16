...
void setup() {
  ...
  _final = createGraphics(width, height, SVG, "maze_oneline.svg");
...
void tryLine(int bx, int by, boolean invert) {
  ...
  while (x<qttX && y<qttY) {
    ...
    if (brF>127 == invert && !isLine) {
      ...
      _final.vertex(x*stepX, y*stepY);
      } else {
      vertex(x*stepX, y*stepY+stepY);
      _final.vertex(x*stepX, y*stepY+stepY);
      }
    ...
    if (isLine && (x == qttX-1 || y == qttY-1)) {
      ...
      _final.vertex(x*stepX+stepX, y*stepY+stepY);
      } else {
      vertex(x*stepX, y*stepY);
      //_final.vertex(x*stepX, y*stepY);
      }

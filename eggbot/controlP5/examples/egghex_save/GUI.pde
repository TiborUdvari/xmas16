...
void setupGUI() {
  ...
  sliders[si++] = controlP5.addSlider("repetition", 1, 6, left, top+posY+20, len, 15);
  posY += 50;
  sliders[si++] = controlP5.addSlider("Hexagon Spacing", 1, 50, left, top+posY, len, 15);
  posY += 20;
...
void controlEvent(ControlEvent theControlEvent) {
  ...
  if (theControlEvent.isController()) {
    ...
    if (theControlEvent.getController().getName().equals("Hexagon Spacing")) {
      float f = theControlEvent.getController().getValue();
      HexSpacing = f;
    }

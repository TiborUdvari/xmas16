...
void draw() {
  ...
  for (int level=1; level<4; level++) {
    ...
    for (int i=0; i<50; i++) {
      ...
      polygon(60*i+level*30, pgDrawing.height/2+up*level*HexSpacing, HexSize, 6);  // Heptagon
      pgDrawing.popMatrix();
    }
    ...
    for (int i=0; i<50; i++) {
      pgDrawing.pushMatrix();
      polygon(60*i+level*30, pgDrawing.height/2+up*level*HexSpacing, 10, 6);  // Heptagon
      pgDrawing.popMatrix();
    }
  }
